// Package docs 把上传的附件转成纯文本，喂给上游模型。
//
// 迁移来源：M365-Copilot2API 的 internal/web/chat_docs.go。
// 上游只吃文本，所以附件必须先在这里落成文本 —— 客户端发来的
// base64 文档本身对模型毫无意义。
//
// 覆盖 PDF / XLSX / CSV / TSV / 纯文本。不做 OCR：扫描件里没有文字层，
// 硬塞给模型只会得到一段无意义内容，不如明确告诉用户不支持。
package docs
...
// Supported 报告这个文件名是否是我们能解析的附件类型。
func Supported(name string) bool { return docSupported(name) }

// Extract 把附件转成纯文本。
//
// 这是本包唯一的对外入口。返回的文本可能被截断（见 capRunes 的调用处），
// 因为一份几百页的 PDF 全塞进上下文只会把窗口挤爆，反而让模型答不好。
func Extract(name, mime string, data []byte) (string, error) {
	return extractDocText(name, mime, data)
}

// MaxBytes 是单个附件的体积上限。
//
// 32MB 与网关的请求体上限对齐：更大的文件在解析阶段就会吃掉大量内存，
// 而 NAS 的内存通常很紧。
